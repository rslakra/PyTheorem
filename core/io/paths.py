...
def getFolderPath(folderId, user_handle=UserHandle.common):
    fid = GUID(folderId)
...
if __name__ == '__main__':
    if len(sys.argv) < 2 or sys.argv[1] in ['-?', '/?']:
        print('python paths.py FolderUUID {current|common}')
        sys.exit(0)

    try:
        folderId = getattr(FolderUUID, sys.argv[1])
    ...
    try:
        if len(sys.argv) == 2:
            print(getFolderPath(folderId))
        else:
            print(getFolderPath(folderId, getattr(UserHandle, sys.argv[2])))
